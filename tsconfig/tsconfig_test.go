...
func TestLoad(t *testing.T) {
	tests := []struct {
		...
		{
			name: "extends from bare package name",
			fsys: fstest.MapFS{
				"node_modules/@acme/tsconfig/tsconfig.json": {Data: []byte(`{
					"compilerOptions": { "moduleResolution": "bundler" }
				}`)},
		...
		{
			name: "references to file and directory",
			fsys: fstest.MapFS{
				"tsconfig.json": {Data: []byte(`{
					"references": [
			...
			path: "tsconfig.json",
			expected: Config{
				References: []string{"tsconfig.app.json", "packages/lib/tsconfig.json"},
			},
		},
		{
			name: "references are not inherited via extends",
			fsys: fstest.MapFS{
				"tsconfig.base.json": {Data: []byte(`{
					"references": [{ "path": "./tsconfig.base-ref.json" }],
					"compilerOptions": { "baseUrl": "." }
				}`)},
				"tsconfig.json": {Data: []byte(`{
					"extends": "./tsconfig.base.json",
			...
			expected: Config{
				BaseURL:    ".",
				References: []string{"tsconfig.app.json"},
			},
		},
	}
