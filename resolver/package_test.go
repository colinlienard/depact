...
func TestResolvePkgEntry(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			fsys: fstest.MapFS{
				"node_modules/broken/package.json": {Data: []byte(`{not json`)},
			},
			specifier: "broken",
			wantErr:   nil, // any non-sentinel error is acceptable; checked below
		},

		// --- exports field: enable as the implementation lands ---
